...
class TestKubeClient:
    ...
    async def test_storage_is_auto_provided(
        ...
        async def wait_for_storage() -> None:
            while True:
                pvc_read = await kube_client.get_pvc(namespace.name, pvc.name)
    ...
    async def test_retrieve(
        ...
        await kube_client.create_pvc(namespace.name, pvc_write)
        pvc_read = await kube_client.get_pvc(namespace.name, pvc_write.name)
    ...
    async def test_retrieve_does_not_exists(
        ...
        with pytest.raises(ResourceNotFound):
            await kube_client.get_pvc(kube_client.namespace, "not-exists")

    async def test_delete_does_not_exists(
        self, kube_client: KubeClient, k8s_storage_class: str
    ) -> None:
        with pytest.raises(ResourceNotFound):
            await kube_client.remove_pvc(kube_client.namespace, "not-exists")
    ...
    async def test_watch_pods(
        ...
        async with kube_client.run_pod(namespace.name, [pvc1.name]):
            await asyncio.sleep(0.5)

        assert pvc1.name in seen_pvc
        assert pvc2.name not in seen_pvc

        async with kube_client.run_pod(namespace.name, [pvc2.name]):
            await asyncio.sleep(0.5)
    ...
    async def test_disk_naming_crud(
        self,
        kube_client: KubeClient,
        scoped_namespace: tuple[Namespace, str, str],
    ) -> None:
